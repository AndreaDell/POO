...
public class ABR<T extends Comparable<? super T>> implements CollezioneOrdinata<T>{
	...
	public  int contaFogliePari(Nodo<T> radice) {
		int cnt=0;
		return contaFogliePari(radice, cnt);
	}
	
	private  int contaFogliePari(Nodo<T> radice, int cnt) {
		if (radice==null) return cnt;
		if (radice.fS==null && radice.fD==null) {
			if ((int)radice.info%2==0) cnt++;
		}else 
			return contaFogliePari(radice.fD,cnt)+contaFogliePari(radice.fS,cnt);
	}
	
	public  int conta(Nodo<T> a, int l, int u) {
		int cnt=0;
		return contaRic(a,l,u,cnt);
		}
			
		private  int contaRic (Nodo<T> a, int l, int u, int cnt) {
		if (a.info==null) return cnt;
		if ((int)a.info>l && (int)a.info<u) cnt++;
		return cnt+contaRic(a.fD,l,u,cnt)+contaRic(a.fS,l,u,cnt);
	...
	public static void main( String[] args ) {
		...
		System.out.println(conta(abr,5,10));
